...
export default function CEOOKRSetup() {
  ...
  // 기존 진행 상태 복원 (기간 선택 + 조직 로딩 완료 후 자동 실행)
  // ★ organizations.length 의존성 — 비동기 조직 로딩 완료 후 재실행 필요
  useEffect(() => {
    ...
    try {
      ...
      if (companyObjs && companyObjs.length > 0) {
        ...
        if (isFinalized) {
          ...
          setCurrentStep(2); // Step 2: 전사 OKR 수립 화면 (확정 완료 상태)
        } else {
          setCurrentStep(2); // Step 2: OKR이 있지만 아직 확정 전
        }
      }
        ...
        if (statuses.length > 0) {
          ...
          if (allDone && statuses.length === childOrgs.length) {
            ...
            if (companyObjs && companyObjs.some((o: any) => o.approval_status === 'finalized')) {
              setCurrentStep(3); // 전사 확정 + 조직 초안 완료 → Step 3 (전체 조직 초안)
            }
          }
        }
      }
