...
export default function OKRExportModal({ isOpen, onClose, defaultPeriodCode }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        // 1. fiscal_periods 테이블에서 로드 (컬럼: period_code, period_name, status)
        const { data } = await supabase
          .from('fiscal_periods')
          .select('period_code, period_name, status')
        ...
        if (data && data.length > 0) {
          list = data.map(p => ({
            code: p.period_code,
            label: p.period_name || p.period_code,
            status: p.status || '',
          }));
        } else {
          // ★ Fallback: fiscal_periods가 없으면 objectives의 period 컬럼에서 직접 추출
          ...
          if (orgIds && orgIds.length > 0) {
            ...
            const { data: objPeriods } = await supabase
              .from('objectives')
              .select('period')
              .in('org_id', ids)
              .not('period', 'is', null);

            const uniquePeriods = [...new Set((objPeriods || []).map((o: any) => o.period as string))]
              .filter(Boolean)
              .sort((a, b) => b.localeCompare(a));

            list = uniquePeriods.map(code => ({
              code,
              label: code,
              status: 'active',
            }));
        ...
        // 기본 선택
        if (list.length > 0) {
          const current = list.find(p => p.code === defaultPeriodCode);
          const active = list.find(p => p.status === 'active');
          const planning = list.find(p => p.status === 'planning');
          setSelectedPeriod(
            current?.code ?? active?.code ?? planning?.code ?? list[0].code,
          );
        }
      } finally {
  ...
  const toggleLevel = (level: string) => {
    ...
    setSelectedOrgIds(prev => {
      const next = new Set(prev);
      allSelected ? orgs.forEach(o => next.delete(o.id)) : orgs.forEach(o => next.add(o.id));
      return next;
    });
  };
  ...
  const statusLabel = (s: string) => {
    const m: Record<string, string> = {
      active: '실행중', planning: '수립중', upcoming: '예정',
      closing: '마감중', closed: '마감', archived: '아카이브',
    };
    return m[s] ?? '';
  };
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            ...
            {loadingPeriods ? (
              ...
              <p className="text-sm text-red-400">등록된 기간이 없습니다. 관리자 설정에서 기간을 먼저 생성해주세요.</p>
            ) : (
              <div className="flex flex-wrap gap-2">
                {periods.map(p => (
                  ...
                  >
                    {p.label}
                    {p.status && (
                      <span className="ml-1.5 text-xs opacity-70">({statusLabel(p.status)})</span>
                    )}
                  </button>
                ))}
              </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={clearAll} className="text-xs text-slate-500 hover:underline">전체 해제</button>
              </div>
            </div>
                ...
                return (
                  <div key={level}>
                    <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 hover:bg-slate-100 transition-colors">
                      ...
                      <button onClick={() => toggleLevel(level)} className="flex items-center gap-1 text-xs text-slate-500 hover:text-blue-600 transition-colors">
                        {allSelected ? (
                    ...
                    {expanded && (
                      <div className="divide-y divide-slate-50">
                        {orgs.map(org => (
                          <label key={org.id} className="flex items-center gap-3 px-5 py-2 hover:bg-blue-50/50 cursor-pointer transition-colors">
                            <input
                              type="checkbox"
            ...
            {selectedCount === 0 && (
              <p className="mt-2 text-xs text-slate-400">※ 조직을 선택하지 않으면 전체 조직이 포함됩니다.</p>
            )}
          </div>
